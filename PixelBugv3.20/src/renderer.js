...
function setVoxelModeSlice(nextSlice, message = "Voxel slice changed.") {
  ...
  renderVoxelMode();
  saveLocal();
  setStatus(`${message} ${voxelModeSliceLabel()}.`);
  return true;
}
...
function syncVoxelModeControls() {
  ...
  if (voxelModeInfo) voxelModeInfo.textContent = `${model.width} × ${model.height} × ${model.depth} model, ${voxelModeMap().size} cube${voxelModeMap().size === 1 ? "" : "s"}. Editing ${model.activePlane.toUpperCase()} ${voxelModeSliceLabel()} independently from the 3D camera. Origin X ${origin.x}, Y ${origin.y}, Z ${origin.z}.`;
...
function voxelModeCameraPoint(x, y, z, model, yaw = voxelModeCameraAngles(model).yaw, pitch = voxelModeCameraAngles(model).pitch) {
  ...
  return {
    x: rx * 24,
    y: (dy * cosPitch - rz * sinPitch) * 24,
    depth: dy * sinPitch + rz * cosPitch
  };
}
...
function voxelModeCubeFace(cube, faceName, model, angle = voxelModeYawAngle(model)) {
  ...
  const faces = {
    bottom: { points: [p.a, p.b, p.c, p.d], fill: shadeHex(cube.color, 18) },
    ...
    top: { points: [p.e, p.f, p.g, p.h], fill: shadeHex(cube.color, -34) }
  };
  const face = faces[faceName] || faces.top;
  return { ...face, cube, faceName, normal: voxelModeFaceNormal(faceName), depth: voxelModeFaceDepth(face.points) };
}
...
function voxelModePreviewCornerBounds(model, yaw = voxelModeYawAngle(model), pitch = voxelModeCameraAngles(model).pitch) {
  ...
  ].map(point => voxelModeCameraPoint(point[0], point[1], point[2], model, yaw, pitch));
...
function voxelModePreviewTarget(hit, erase = false) {
  if (!hit || hit.type === "background") return null;
...
function drawVoxelModeHover(transform, model, angle) {
  if (!voxelModePreviewHover || voxelModePreviewHover.type === "background") return;
  const erase = voxelModel().tool === "erase" || voxelModePreviewHover.erase === true;
  const target = voxelModePreviewTarget(voxelModePreviewHover, erase);
  if (!voxelModeTargetInBounds(target)) return;
  if (erase || voxelModel().tool === "pick") {
    ...
    voxelModePreviewCtx.fillStyle = erase ? "rgba(255,70,70,0.28)" : "rgba(255,255,255,0.22)";
  ...
  if (voxelModel().tool !== "paint" || voxelModeGetCube(target.x, target.y, target.z)) return;
  const cube = { ...target, color, w: 1, h: 1, d: 1 };
...
function drawVoxelModePreview() {
  ...
  drawVoxelModeActivePlane(voxelModePreviewPlane);
  if (!cubeCount) {
    voxelModePreviewFaces = [];
    voxelModePreviewCtx.fillStyle = canvasMutedColor();
  ...
  voxelModePreviewFaces = faces.map(face => {
    const screenPoints = drawVoxelModePoly(voxelModePreviewCtx, face.points, face.fill, transform);
  ...
  voxelModePreviewCtx.font = `800 ${Math.max(12, voxelModePreviewCanvas.width / 55)}px system-ui, sans-serif`;
  const viewText = model.turntable ? "Turntable" : voxelModeViewName(model.cameraPreset);
  voxelModePreviewCtx.fillText(`${model.width}×${model.height}×${model.depth} • ${cubeCount} cubes • ${faces.length} camera-facing surfaces • ${viewText}`, 16, voxelModePreviewCanvas.height - 16);
}
...
function applyVoxelModePreviewEdit(hit, eraseOverride = false) {
  ...
  if (!["paint", "erase", "pick"].includes(model.tool) && !eraseOverride) return false;
  const target = voxelModePreviewTarget(hit, erase || pick);
  ...
  if (pick) {
    ...
    renderPalette();
    renderVoxelModePalette();
    setStatus(`Picked ${cube.color} from X ${cube.x}, Y ${cube.y}, Z ${cube.z}.`);
    return false;
  }
  const targetKey = `${erase ? "erase" : "paint"}:${target.x},${target.y},${target.z}`;
  if (targetKey === voxelModePreviewLastTarget) return false;
  voxelModePreviewLastTarget = targetKey;
  const coords = erase ? [target] : voxelModePreviewBrushCoords(target);
  const changed = erase
    ? voxelModeApplyCoords(coords, item => voxelModeDeleteCube(item.x, item.y, item.z))
    : voxelModeApplyCoords(coords, item => voxelModeSetCube(item.x, item.y, item.z, color));
  if (changed) {
    voxelModePreviewHover = null;
...
function beginVoxelModePreviewInteraction(event) {
  ...
  if (event.button === 0 && !["paint", "erase", "pick"].includes(model.tool)) {
    setStatus("Direct 3D editing supports Paint, Erase, and Pick. Use the slice editor for the selected tool.");
    return;
  }
  event.preventDefault();
...
function endVoxelModePreviewInteraction(event) {
  ...
  if (mode === "edit") {
    ...
    if (changed || voxelModeStrokeChanged) {
      saveLocal();
      setStatus(voxelModePreviewInteraction.erase || voxelModel().tool === "erase" ? "Erased voxels in the 3D view." : "Painted voxels in the 3D view.");
    }
  } else {
    saveLocal();
...
function importVoxelPreviewToVoxelMode() {
  ...
  state.voxelModel = {
    ...
    viewYaw: Number(previousModel.viewYaw) || 0,
    cameraPreset: Number.isInteger(Number(previousModel.cameraPreset)) ? Number(previousModel.cameraPreset) : 6,
    cameraYaw: Number.isFinite(Number(previousModel.cameraYaw)) ? Number(previousModel.cameraYaw) : Math.PI / 4,
    cameraPitch: Number.isFinite(Number(previousModel.cameraPitch)) ? Number(previousModel.cameraPitch) : -Math.PI / 6,
    cameraZoom: Math.max(0.35, Math.min(Number(previousModel.cameraZoom) || 1, 5)),
    cameraPanX: Number(previousModel.cameraPanX) || 0,
    cameraPanY: Number(previousModel.cameraPanY) || 0,
    turntable: false,
...
async function importVoxelModeJSON() {
  try {
    ...
    renderVoxelMode();
    saveLocal();
    setStatus(`Opened voxel model with ${voxelModeMap().size} cube${voxelModeMap().size === 1 ? "" : "s"}.`);
  } catch (error) {
...
function buildVoxelModeOBJ() {
  ...
  const colors = Array.from(new Set(descriptors.map(item => item.cube.color)));
  const vertices = [];
  ...
  const facePoints = (cube, faceName) => {
    const x0 = cube.x, x1 = cube.x + 1;
    const y0 = model.height - cube.y - 1, y1 = model.height - cube.y;
    const z0 = cube.z, z1 = cube.z + 1;
    const points = {
      bottom: [[x1,y1,z0],[x1,y1,z1],[x0,y1,z1],[x0,y1,z0]],
      top: [[x0,y0,z0],[x0,y0,z1],[x1,y0,z1],[x1,y0,z0]],
      back: [[x0,y0,z0],[x1,y0,z0],[x1,y1,z0],[x0,y1,z0]],
      front: [[x0,y0,z1],[x0,y1,z1],[x1,y1,z1],[x1,y0,z1]],
      right: [[x1,y0,z0],[x1,y0,z1],[x1,y1,z1],[x1,y1,z0]],
      left: [[x0,y0,z0],[x0,y1,z0],[x0,y1,z1],[x0,y0,z1]]
    };
    return points[faceName] || points.front;
  };
  descriptors.forEach(({ cube, faceName }) => {
    const indices = facePoints(cube, faceName).map(vertexIndex);
    if (!facesByColor.has(cube.color)) facesByColor.set(cube.color, []);
    facesByColor.get(cube.color).push(indices);
  });
  vertices.forEach(vertex => obj.push(`v ${vertex[0]} ${vertex[1]} ${vertex[2]}`));
  colors.forEach(cubeColor => {
    obj.push(`usemtl mat_${cubeColor.slice(1)}`);
    facesByColor.get(cubeColor).forEach(indices => obj.push(`f ${indices.join(" ")}`));
  });
  ...
  return { obj: obj.join("\n"), mtl, faceCount: descriptors.length, vertexCount: vertices.length };
}
async function exportVoxelModeOBJ() {
  ...
  setStatus(res.ok ? `Voxel OBJ exported with ${data.faceCount} visible faces and ${data.vertexCount} shared vertices.` : "Save cancelled.");
}
...
if (voxelModePreviewCanvas) {
  ...
  voxelModePreviewCanvas.addEventListener("wheel", event => {
    event.preventDefault();
    zoomVoxelModeCamera(event.deltaY < 0 ? 1.12 : 1 / 1.12);
  }, { passive: false });
  voxelModePreviewCanvas.addEventListener("dblclick", resetVoxelModeCamera);
}
if (voxelPaintCanvas) voxelPaintCanvas.addEventListener("wheel", event => {
  event.preventDefault();
  moveVoxelModeSlice(event.deltaY < 0 ? -1 : 1);
}, { passive: false });

...
(() => {
  ...
  function interactionCharacterLabel(id) {
    if (!id || id === "any") return "any character";
    const options = api?.getInteractionCharacterOptions?.() || [];
  ...
  function nodeStatement(node) {
    ...
    if (node.type === "eventInteract") return `WHEN player interacts with ${interactionCharacterLabel(node.data.character)}`;
  ...
  function nodeDetail(node) {
    ...
    if (node.type === "eventInteract") return node.data.character === "any" ? "Runs when the player interacts with any character." : `Runs when the player interacts with ${interactionCharacterLabel(node.data.character)}.`;
  ...
  function computeGraphIssues(data = graph()) {
    ...
    const interactionOptions = new Set(["any", ...(api?.getInteractionCharacterOptions?.() || []).map(option => String(option.id || ""))]);
    ...
    nodes.forEach(node => {
      ...
      if (node.type === "eventInteract" && node.data.character !== "any" && !interactionOptions.has(String(node.data.character || ""))) issues.push({ id: node.id, text: `${node.name} uses a character that is no longer available for interaction.` });
  ...
  function interactionOptionsMarkup(current) {
    const options = [{ id: "any", label: "Any character" }, ...(api?.getInteractionCharacterOptions?.() || [])];
  ...
  function fieldMarkup(node, prefix) {
    ...
    if (node.type === "eventInteract") return `<label class="node-editor-field" for="${prefix}-character"><span>Character to Interact With</span><select id="${prefix}-character" data-field="character" aria-label="Character that starts this rule">${interactionOptionsMarkup(node.data.character)}</select></label><p class="control-hint">This rule starts when the player presses Interact near the chosen character or an object assigned to them.</p>`;
  ...
  function connectNodes(sourceId, port, targetId) {
    ...
    api.saveLocal();
    api.setStatus(`${portTitle(port)} connection created from ${source.name} to ${target.name}.`);
    return true;
  }
  ...
  function armPendingConnection(id, port, ctx, sourceElement) {
    ...
    ctx.board.setAttribute("aria-label", `Connecting ${portTitle(port)} from ${source.name}. Click another rule or press Escape to cancel.`);
    api.setStatus(`${portTitle(port)} connection ready. Click the rule that should run next.`);
  }
  ...
  function startConnection(event, id, port) {
    ...
    sourceElement?.classList.add("connection-source", `connection-${port}`);
    ctx.board.classList.add("connecting", "node-editor-board-live");
    ctx.board.setAttribute("aria-label", `Connecting ${portTitle(port)} from ${source.name}. Drag to another rule or release to use click connection.`);
  ...
  function nodeCountForCharacter(characterId) {
    return graph().nodes.filter(node => node.type === "eventInteract" && String(node.data.character || "any") === String(characterId || "")).length;
  }
  ...
  function createInteractionForCharacter(option = {}) {
    ...
    const data = graph();
    const existing = data.nodes.find(node => node.type === "eventInteract" && String(node.data.character || "any") === String(option.id));
    if (existing) {
      select(existing.id);
    ...
    const eventNode = normalizeNode({ id: makeId("interact"), type: "eventInteract", name: `Interact with ${String(option.name || option.id).slice(0, 28)}`, x: eventPosition.x, y: eventPosition.y, data: { character: option.id } }, data.nodes.length);
  ...
  function renderContext(ctx) {
    ...
    if (pendingHere) ctx.board.setAttribute("aria-label", `Connecting ${portTitle(pendingConnection.port)}. Click another rule or press Escape to cancel.`);
    makeActions(ctx);
    renderMapTools(ctx);
    renderLessons(ctx);
    renderBindings(ctx);
  ...
  function runEvent(type, payload = {}) {
    ...
    if (nodes.length) {
      const message = type === "triggerEnter" ? `Player touched ${payload?.name || "an object"}.` : type === "characterInteract" ? `Player interacted with ${payload?.name || "a character"}.` : "Game start rules began.";
      logRuntime(message);
  ...
  window.PixelBugNodeEditor = { mount, render, runEvent, syncPlayMode, resetRuntime, createTriggerForObject, createInteractionForCharacter, createTextEventNode, attachTextEventToSelectedNode, openOverlay };

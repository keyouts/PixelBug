...
window.addEventListener("keydown", e => { const editable=["INPUT","TEXTAREA","SELECT"].includes(e.target?.tagName); if(editable && !(e.ctrlKey||e.metaKey) && e.key !== "Escape") return; if(e.key === "Escape" && selectionBox && (pixelizerOverlay.hidden && voxelOverlay.hidden && customSizeOverlay.hidden && privacyOverlay.hidden && (!newProjectOverlay || newProjectOverlay.hidden))) { e.preventDefault(); selectionAction("clear"); return; } if(e.key === "Escape" && !pixelizerOverlay.hidden) { closePixelizerModal(); return; } if(e.key === "Escape" && !voxelOverlay.hidden) { closeVoxelModal(); return; } if(e.key === "Escape" && !customSizeOverlay.hidden) { closeCustomSizeModal(); return; } if(e.key === "Escape" && !privacyOverlay.hidden) { closePrivacyModal(); return; } if(e.key === "Escape" && newProjectOverlay && !newProjectOverlay.hidden) { closeNewProjectConfirm(); return; } const mod=e.ctrlKey||e.metaKey; if(mod&&e.key.toLowerCase()==="s"){e.preventDefault();saveProject();} if(mod&&e.key.toLowerCase()==="o"){e.preventDefault();openProject();} if(mod&&e.key.toLowerCase()==="z"){e.preventDefault(); e.shiftKey ? $("#redo-btn").click() : $("#undo-btn").click();} if(mod&&e.key.toLowerCase()==="y"){e.preventDefault();$("#redo-btn").click();} if(mod&&e.key.toLowerCase()==="c"){e.preventDefault();selectionAction("copy");}
...
const map={b:"pencil",e:"eraser",g:"fill",i:"eyedropper",l:"line",r:"rect",o:"ellipse",m:"select",t:"text"}; if(!mod && map[e.key.toLowerCase()]) setTool(map[e.key.toLowerCase()]); });

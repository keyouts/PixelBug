...
(() => {
  ...
  function drawLine(ctx, x1, y1, x2, y2, faded = false, active = false) {
    const line = document.createElement("div");
    const length = Math.hypot(x2 - x1, y2 - y1);
    const angle = Math.atan2(y2 - y1, x2 - x1) * 180 / Math.PI;
    line.className = `node-editor-link-line ${faded ? "alt" : ""} ${active ? "active-route" : ""}`;
  ...
  function drawLinks(ctx) {
    ...
    byId.forEach(node => {
      [[node.next, false], [node.alt, true]].forEach(([targetId, faded]) => {
        ...
        if (!target) return;
        drawLine(ctx, node.x + 188, node.y + 46, target.x, target.y + 46, faded, node.id === selectedId || target.id === selectedId);
      });
    });
  ...
  function renderRuntime(ctx) {
    ...
    const items = runtimeLog.length ? runtimeLog.map(item => `<li>${escape(item)}</li>`).join("") : `<li>Press Run in the top tester, touch a bound object, or use Test Selected here.</li>`;
    ctx.runtime.innerHTML = `<div class="node-editor-section-head"><strong>Tester Log</strong><span>${escape(playTesterStateLabel())}</span></div><ul>${items}</ul><p class="sr-only">This log updates when Play Mode fires node events and actions.</p>`;
  }
  ...
  function renderNodes(ctx) {
    ...
    graph().nodes.forEach(node => {
      ...
      item.className = `node-editor-node ${node.id === selectedId ? "active" : ""} ${EVENT_TYPES.has(node.type) ? "event" : ""} ${isActive(node.id) ? "running" : ""} ${node.id === highlightedId ? "just-created" : ""} ${node.id === settlingId ? "settling" : ""} ${node.type === "eventTrigger" && node.data.trigger && node.data.trigger !== "any" ? "bound" : ""}`;
      item.style.left = `${Math.round(node.x)}px`;
      item.style.top = `${Math.round(node.y)}px`;
      item.dataset.nodeId = node.id;
      item.tabIndex = 0;
      item.setAttribute("role", "button");
      item.setAttribute("aria-label", `${label(node)} node. Drag to move. Use Next or Alt to connect.`);
      ...
      flow.textContent = routeSummary(node);
      body.append(title, type, routes, flow);
  ...
  function runSelectedNode() {
    ...
    if (EVENT_TYPES.has(node.type)) {
      pulseNode(node.id);
      logRuntime(`Tested ${node.name}.`);
      executeNode(node.next || node.alt, {}, new Set([node.id]));
    } else {
  ...
  function makeActions(ctx) {
    ...
    ctx.actions.append(
      makeAddMenu(ctx),
      button("Bind Selected Object", "create or attach a Trigger Enter node for the selected Play Mode object", attachSelectedObject),
      button("Test Selected", "test the selected node chain in the top Play Mode canvas", runSelectedNode),
      button("Test Scene Start", "fire the Scene Start event without starting the full tester", runSceneStart),
      button("Wrangle Nodes", "repair bindings, add missing playable routes, and arrange the node map", wrangleGraph),
      button("Clear Links", "clear selected node connections", clearConnection),
      button("Delete", "delete selected node", deleteNode),
      button("Reset", "reset node graph", resetGraph)
    );
    if (ctx.key === "inline") ctx.actions.append(button("Large View", "open large node editor", openOverlay));
  }
  ...
  function buildInline() {
    ...
    if (!card) {
      ...
      card.innerHTML = `<h3><span class="drag-handle" aria-hidden="true">↕</span> Play Mode Logic</h3><p class="control-hint" id="node-editor-help">Build the logic that powers the tester at the top. Start with Scene Start or bind a scene object, then connect actions to see them happen on the Play Mode canvas.</p><div class="node-editor-guide"></div><div class="button-row action-row play-actions node-editor-actions"></div><div class="node-editor-bindings" aria-label="Play Mode object bindings"></div><div class="node-editor-runtime" aria-live="polite"></div><div class="node-editor-workspace"><div class="node-editor-board" role="application" tabindex="0" aria-label="Visual Play Mode node graph" aria-describedby="node-editor-help"><div class="node-editor-nodes"></div></div><div class="node-editor-inspector" aria-label="Selected node inspector"></div></div>`;
  ...
  function buildOverlay() {
    ...
    if (!overlay) {
      ...
      overlay.hidden = true;
      overlay.innerHTML = `<div class="modal-card node-editor-large" role="dialog" aria-modal="true" aria-labelledby="node-editor-large-title"><div class="modal-head"><h2 id="node-editor-large-title">Play Mode Logic</h2><button type="button" class="node-editor-close" aria-label="Close large node editor">Close<span class="sr-only"> large node editor</span></button></div><p class="control-hint" id="node-editor-large-help">Large graph view for the Play Mode tester. Run above fires Scene Start; touching bound objects fires Trigger Enter; actions show on the top canvas.</p><div class="node-editor-guide"></div><div class="button-row action-row play-actions node-editor-actions"></div><div class="node-editor-bindings" aria-label="Large Play Mode object bindings"></div><div class="node-editor-runtime" aria-live="polite"></div><div class="node-editor-workspace"><div class="node-editor-board node-editor-large-board" role="application" tabindex="0" aria-label="Large visual Play Mode node graph" aria-describedby="node-editor-large-help"><div class="node-editor-nodes"></div></div><div class="node-editor-inspector" aria-label="Large selected node inspector"></div></div></div>`;
  ...
  function executeNode(id, payload = {}, seen = new Set()) {
    ...
    if (node.type === "actionFinish") api.finishPlayMode?.(node.data.message || "Finished.");
    if (node.type === "actionDialogue") api.startPlayDialogue?.(Math.max(0, Number(node.data.line) || 0));
    if (node.type === "actionCheckpoint") api.setPlayCheckpoint?.();
    if (node.type === "actionMoveActor") api.movePlayActor?.(Number(node.data.dx) || 0, Number(node.data.dy) || 0);
    if (node.type === "actionSetVariable") data.runtime.variables[node.data.variable] = String(node.data.value ?? "true");
    if (node.type === "logicVariable") nextId = String(data.runtime.variables[node.data.variable] ?? "") === String(node.data.equals) ? node.next : node.alt;
    if (nextId) window.setTimeout(() => executeNode(nextId, payload, seen), 0);
  }
  ...
  function runEvent(type, payload = {}) {
    ...
    nodes.forEach(node => { pulseNode(node.id); executeNode(node.next || node.alt, payload, new Set([node.id])); });
  }

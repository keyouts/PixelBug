...
function drawDialogueCharacters() {
  ...
  dialogue.characters.filter(character => character.role !== "actor" && character.visible !== false).forEach(character => {
    ...
    const x = Math.round(playCanvas.width * (slots[character.position] || 0.5) - metrics.w / 2);
...
function startPlayDialogueAtLine(lineIndex = 0) {
  normalizeProject();
...
const nodeVariables = Object.assign({}, (nodeGraph.runtime && nodeGraph.runtime.variables) || {});
...
function startDialogueAtLine(index) { dialogueActive = true; activeCharacterId = "node-editor"; dialogueLine = Math.max(0, Math.min(Number(index) || 0, Math.max(0, lines().length - 1))); revealStart = performance.now(); revealChars = 0; }
...
function resetActorToCheckpoint() { const ar = actorRect(); const cp = nodeCheckpoint || (nodeGraph.runtime && nodeGraph.runtime.checkpoint) || null; actor.x = cp && Number.isFinite(Number(cp.x)) ? Number(cp.x) : 24; actor.y = cp && Number.isFinite(Number(cp.y)) ? Number(cp.y) : groundY() - ar.h; actor.vx = 0; actor.vy = 0; actor.grounded = true; gameFinished = false; clampActor(); updateCamera(); }
// Node runtime
...
function executeNode(id, seen) {
  ...
  if (node.type === "actionMessage") showNodeMessage(Number(data.textLine) >= 0 ? textLineMessage(data.textLine) || data.message || node.name : data.message || node.name || "Message");
  if (node.type === "actionDialogue") startDialogueAtLine(data.line);
  ...
  if (nextId) window.setTimeout(() => executeNode(nextId, seen), 0);
}
function runNodeEvent(type, payload) {
  (nodeGraph.nodes || []).filter(node => {
    ...
    return false;
  }).forEach(node => executeNode(node.next || node.alt, new Set([String(node.id)])));
}
function syncNodeTriggers() {
  ...
  props.forEach((prop, index) => {
    ...
    if (!nodeEntered.has(triggerId)) runNodeEvent("triggerEnter", { id:triggerId, name:triggerId, ids, prop });
  });
  nodeEntered = entered;
}

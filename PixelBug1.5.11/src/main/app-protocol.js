...
const APP_FILE_TYPES = new Set([".html", ".js", ".css"]);
...
function registerHandler(srcRoot) {
  protocol.handle(SCHEME, request => {
    ...
    if (url.hostname === APP_HOST && APP_FILE_TYPES.has(path.extname(relativePath).toLowerCase())) return net.fetch(pathToFileURL(targetPath(srcRoot, relativePath)).toString());

...
(() => {
  ...
  function defaultDataForType(type) {
    if (type === "eventStart") return { sceneId: api?.getState?.()?.playMode?.activeSceneId || "" };
  ...
  function lessonNode(type, name, x, y, data = {}) {
    const nextData = type === "eventStart" && !data.sceneId ? { ...data, sceneId: api?.getState?.()?.playMode?.activeSceneId || "" } : data;
  ...
  function createAudioAssignment(option = {}) {
    ...
    const eventData = eventType === "eventTrigger" ? { trigger: target } : eventType === "eventInteract" ? { character: target } : { sceneId: target };
  ...
  window.PixelBugNodeEditor = { mount, render, runEvent, syncPlayMode, resetRuntime, createTriggerForObject, createInteractionForCharacter, createTextEventNode, attachTextEventToSelectedNode, createAudioAssignment, getAudioAssignments, openOverlay, getSummary };

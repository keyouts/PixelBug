...
function activeFrameSignature(index = state.activeFrame) {
  ...
  if (!f) return "missing";
  return `${index}:${projectWidth()}:${projectHeight()}:${f.layers.length}:${f.layers.map(layer => `${layer.visible ? 1 : 0}:${Number(layer.opacity ?? 1).toFixed(3)}:${layer.name || ""}`).join("|")}`;
}
...
function normalizeProject() {
  ...
  state.playMode.dialogue.lines = state.playMode.dialogue.lines.map(line => {
    const matched = state.playMode.dialogue.characters.find(character => character.id === line.characterId || character.name === line.speaker) || fallbackCharacter;
    return {
...
function saveDialogueEdits() {
  if (!dialogueSpeakerInput || !state.playMode?.dialogue) return;
  const line = currentDialogueLine();
  const selectedCharacter = dialogueCharacterById(dialogueCharacterSelect?.value);
  line.characterId = selectedCharacter.id;
  line.speaker = String(dialogueSpeakerInput.value || selectedCharacter.name || "Character").slice(0, 32);
  line.text = String(dialogueTextInput.value || "").slice(0, 260);
  line.frame = Math.max(0, Math.min(Number(dialogueFrameSelect?.value ?? selectedCharacter.frame) || 0, state.frames.length - 1));
  state.playMode.dialogue.enabled = dialogueEnabledInput.checked;
  saveLocal();
  drawPlayScene();
}
...
function addDialogueLine(duplicate = false) {
  ...
  saveDialogueEdits();
  const character = state.playMode.dialogue.characters[selectedDialogueCharacter] || state.playMode.dialogue.characters[0];
  const source = duplicate ? currentDialogueLine() : { characterId: character.id, speaker: character.name, text: "New dialogue line.", frame: character.frame };
  state.playMode.dialogue.lines.splice(state.playMode.dialogue.currentLine + 1, 0, clone(source));
  state.playMode.dialogue.currentLine += 1;
  normalizeProject();
  resetDialogueTypewriter();
  renderDialogueControls();
  drawPlayScene();
...
function deleteDialogueLine() {
  if (state.playMode.dialogue.lines.length <= 1) return;
  pushHistory();
  state.playMode.dialogue.lines.splice(state.playMode.dialogue.currentLine, 1);
  state.playMode.dialogue.currentLine = Math.min(state.playMode.dialogue.currentLine, state.playMode.dialogue.lines.length - 1);
  resetDialogueTypewriter();
  renderDialogueControls();
  drawPlayScene();
  saveLocal();
...
.stage { border:4px solid var(--edge); box-shadow:8px 8px 0 var(--edge); background:#fff; width:100%; image-rendering:pixelated; }
canvas { display:block; width:100%; image-rendering:pixelated; outline:none; }
...
<div class="stage"><canvas id="game" tabindex="0" aria-label="Tiny exported Pixel Bug game"></canvas></div>
<p class="help"><kbd>A</kbd>/<kbd>D</kbd> or arrows move. <kbd>W</kbd>/<kbd>Space</kbd> jumps. <kbd>E</kbd>/<kbd>Enter</kbd> talks or advances dialogue.</p>
</main>
<script>
...
const backgroundLayers = visualLayers.filter(layer => layer && layer.role === "background");
const foregroundLayers = visualLayers.filter(layer => layer && (layer.role === "foreground" || layer.role === "overlay"));
...
canvas.width = Math.max(160, Math.min(pm.sceneWidth || 640, 4096));
canvas.height = Math.max(120, Math.min(pm.sceneHeight || 360, 2160));
ctx.imageSmoothingEnabled = false;
function loadImages() {
...
function scale() { return Math.max(1, Math.min(Number(pm.actorScale) || 3, 16)); }
...
function drawLayer(layer) {
  ...
  const px = Math.round((cameraX * (Number(layer.parallax) || 0)) % w);
  ctx.save();
  ctx.globalAlpha = Math.max(0, Math.min(Number(layer.opacity ?? 1), 1));
  if (layer.repeatX !== false) for (let x = -px - w; x < canvas.width + w; x += w) ctx.drawImage(img, x, y, w, h);
  else ctx.drawImage(img, Math.round(-cameraX * (Number(layer.parallax) || 0)), y, w, h);
  ctx.restore();
}
function drawTiledImage(img, y, sc, parallax, opacity = 1) {
  const w = Math.max(1, Math.round(img.width * sc));
  const h = Math.max(1, Math.round(img.height * sc));
  const px = Math.round((cameraX * parallax) % w);
  ctx.save();
  ctx.globalAlpha = Math.max(0, Math.min(Number(opacity ?? 1), 1));
  for (let x = -px - w; x < canvas.width + w; x += w) ctx.drawImage(img, x, Math.round(y), w, h);
  ctx.restore();
}
function drawBackground() {
  ...
  if (img) {
    ...
    if (mode === "stretch") ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
      ...
      const x = Math.round((canvas.width - w) / 2 - cameraX * 0.08);
      const y = Math.round((canvas.height - h) / 2);
      ctx.drawImage(img, x, y, w, h);
      if (x > 0) ctx.drawImage(img, x - w, y, w, h);
      if (x + w < canvas.width) ctx.drawImage(img, x + w, y, w, h);
    }
  }
  backgroundLayers.forEach(drawLayer);
...
function drawDialogue() {
  ...
  revealChars = Math.min(text.length, Math.max(revealChars, Math.floor((performance.now() - revealStart) / 22)));
  ...
  const words = text.slice(0, revealChars).split(/\s+/).filter(Boolean);
  const wrapped = [];
  let row = "";
  words.forEach(word => { const test = row ? row + " " + word : word; if (ctx.measureText(test).width > canvas.width - textX - margin - 16 && row) { wrapped.push(row); row = word; } else row = test; });
...
function draw() {
  ...
  foregroundLayers.forEach(drawLayer);
  drawDialogue();
}
function step() {
  const s = scale();
  ...
  if (pm.autoScroll === true) actor.x = Math.min(worldWidth() - w, actor.x + Math.max(0, Number(pm.scrollSpeed) || 0));
  if (pm.cameraFollow !== false) cameraX = Math.max(0, Math.min(worldWidth() - canvas.width, actor.x + w / 2 - canvas.width / 2));
  tick++;
  draw();
...
[dialogueCharacterSelect, dialogueSpeakerInput, dialogueTextInput, dialogueFrameSelect].filter(Boolean).forEach(input => {
  input.oninput = () => { saveDialogueEdits(); };
});
...
if (dialogueAddBtn) dialogueAddBtn.onclick = () => addDialogueLine(false);

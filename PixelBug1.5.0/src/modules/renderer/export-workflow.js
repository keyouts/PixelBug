(() => {
  ...
  function create(options) {
    ...
    function buildSpritesheet(profile = profileFromControls()) {
      ...
      const meta = { app: "Pixel Bug", version: "1.5.0", image: `${baseName}-sheet.png`, format: "RGBA8888", size: { w: sheet.width, h: sheet.height }, scale, clip: { name: clip.name, start: clip.start, end: clip.end, loop: clip.loop } };
    ...
    function setup() {
      ...
      document.querySelector("#export-batch-btn")?.addEventListener("click", exportBatch);
      document.querySelector("#export-apng-btn")?.addEventListener("click", () => runExportAction(exportAPNG));
      document.querySelector("#export-webp-btn")?.addEventListener("click", () => runExportAction(exportWebP));
    }

...
const { registerHandler, registerScheme, MOD_URL } = require("./main/app-protocol");
const { writeFilesTransaction } = require("./main/file-transactions");
const { ModRunnerManager } = require("./main/mod-runner-manager");
const { clearRecovery, loadRecovery, saveRecovery } = require("./main/recovery-store");
...
const SAFE_INDEX_URL = `${INDEX_URL}?safeRecovery=1`;
...
function hardenWebContents(contents) {
  ...
  contents.on("zoom-changed", (event, direction) => {
    ...
    resetPageZoom(contents);
    contents.send("browser-zoom-blocked", direction === "in" ? "in" : direction === "out" ? "out" : "reset");
  });
...
ipcMain.handle("decode-psd-template", (event, data) => {
  if (!isTrustedSender(event)) throw new Error("Untrusted sender");
...
ipcMain.handle("open-recent-project", async (event, filePath) => {
  if (!isTrustedSender(event)) throw new Error("Untrusted sender");
  return openRecentProject(app, filePath, MAX_OPEN_TEXT_BYTES);
});
